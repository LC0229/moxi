...
def generate_single_doc(
    ...
    file_name: str = "README_BY_MOXI.md",
    cache_dir: Optional[str] = None,
    doc_generator: Optional[OpenAIDocGenerator] = None,
) -> Optional[Dict]:
    """
    ...
    Args:
        ...
        file_name: Name of the file to write (default: "README_BY_MOXI.md")
        cache_dir: Optional cache directory for cloned repos
        doc_generator: Optional OpenAIDocGenerator instance
                     (if None, creates a new one)

    Returns:
        Dictionary with generated documentation and metadata, or None if failed
    """
    try:
        logger.info("Generating documentation", url=repo_url, auto_write=auto_write)
        ...
        # Step 2: Generate documentation using GPT-4o-mini
        if doc_generator is None:
            doc_generator = OpenAIDocGenerator()

        readme_content = doc_generator.generate(repo_analysis)

        if not readme_content:
            logger.warning("Failed to generate documentation", url=repo_url)
            return None
        ...
        logger.info("Documentation generated", url=repo_url, written=auto_write, file_name=file_name)
        return {
            "repo_url": repo_url,
            "readme_content": readme_content,
            "file_name": file_name,
        ...
        logger.error("Failed to generate documentation", url=repo_url, error=str(e))
        return None
...
def generate_batch_docs(
    ...
    """
    Generate documentation for multiple repositories in batch (with optional concurrency).
    ...
    # Create a single doc generator to reuse (more efficient)
    doc_generator = OpenAIDocGenerator()
    ...
    if concurrent:
        ...
        for i, url in enumerate(repo_urls, 1):
            ...
            result = generate_single_doc(url, auto_write, file_name, cache_dir, doc_generator)
            if result:
                results.append(result)
        return results
